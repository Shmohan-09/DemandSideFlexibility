...
def plot_graphs(time_axis, community_battery, prices, solar_to_community, low_price_charge_community, community_battery_discharge,
    ...
    # solar_net_cons = make_subplots(specs=[[{"secondary_y": True}]])
    # solar_net_cons.add_trace(go.Scatter(x=time_axis, y=solar_data_multi_home[:,house_plot-1], name = 'Solar production', line=dict(color='red', width = 0), fill='tozeroy'))
    # solar_net_cons.add_trace(go.Scatter(x=time_axis, y=solar_to_community.value[:,house_plot-1]+solar_charge.value[:,house_plot-1], name = 'Solar consumption', line=dict(color='gold', width = 0), fill='tozeroy'))
    # solar_net_cons.add_trace(go.Scatter(x=time_axis, y=prices/1000, name = 'Prices', line=dict(color='#8c564b')), secondary_y=True)
    # solar_net_cons.update_yaxes(title_text="Power in kW", secondary_y=False)
    # solar_net_cons.update_yaxes(title_text="Price Level in €/kWh", secondary_y=True)
    # fig_format(solar_net_cons)
    # solar_net_cons.update_layout(title = f'Net solar consumption for house {house_plot}')
    ...
    community_soc.update_yaxes(title_text="SoC in kWh", secondary_y=False, range = [0, max(community_battery.value)+10])
    ...
    community_status.update_yaxes(title_text="Power in kW", secondary_y=False, range = [0,80])
    ...
    load_change_grid_system.update_yaxes(title_text="Net grid load in kW", secondary_y=False, range=[-50,110])#
