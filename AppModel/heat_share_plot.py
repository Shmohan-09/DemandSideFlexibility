...
fig_format(fig)
fig.write_image(f"{filename_cwd}/figures/fig14_{house_number}.pdf")
...
fig_format(fig1)
fig1.write_image(f"{filename_cwd}/figures/fig15_{house_number}.pdf")
...
bangbangfig = make_subplots(specs=[[{"secondary_y": True}]])
bangbangfig.add_trace(go.Scatter(x=time_axis, y=bang_bang_control_temp[:,house_number], name = 'Temperature'))
bangbangfig.add_trace(go.Scatter(x=time_axis, y=T_upper_bound[:,house_number], name='Upper bound'), secondary_y=False)
bangbangfig.add_trace(go.Scatter(x=time_axis, y=T_lower_bound[:,house_number], name='Lower bound'), secondary_y=False)
bangbangfig.add_trace(go.Scatter(x=time_axis, y=T_set[:,house_number], name='Set Point'), secondary_y=False)
bangbangfig.add_trace(go.Scatter(x=time_axis, y=bang_bang_control_status[:,house_number],  name = 'Bang bang Status'), secondary_y=True)
bangbangfig.update_yaxes(title_text="Temperature in °C", secondary_y=False)
bangbangfig.update_yaxes(title_text="Power in kW", secondary_y=True)
bangbangfig.update_layout(title = 'Bang-bang control')
bangbangfig.update_layout(autosize=False, width=1000, height=500)
...
with plots:
    ...
    st.plotly_chart(bangbangfig)



carbon_intensity = np.full((4,), carbon_intensity_gas)
carbon_intensity[0] = carbon_intensity_electricity

total_power_consumption = (cp.sum(heater_status, axis = 0)/4).value
total_grid_power_consumption = (cp.sum(heater_status - solar_charge, axis = 0)/4).value
solar_energy_consumption = (cp.sum(solar_charge, axis = 0)/4).value
total_solar_power_consumption = (cp.sum(solar_charge, axis = 0)/4).value
energy_consumption_cost = (prices@(heater_status - solar_charge)/4).value/1000
solar_bang_bang_consumption = (cp.sum(bang_bang_control_solar, axis = 0)/4).value
solar_production = np.sum(solar_data_multi_home, axis = 0)/4
st.write('Independent heating system results')
for house_number in range(4):
    st.write(f'Total heater grid energy consumption house {house_number}: {np.round(total_grid_power_consumption[house_number], 2)} kWh')
    st.write(f'Total energy consumption cost house {house_number}: {np.round(energy_consumption_cost[house_number], 2)} €')
    st.write(f'Total energy consumption house {house_number}: {np.round(total_power_consumption[house_number], 2)} €')
    st.write(f'Solar consumption % solar {house_number}: {np.round(100*total_solar_power_consumption[house_number]/solar_production[house_number], 2)}')
    st.write(f'Solar consumption % home {house_number}: {np.round(100*total_solar_power_consumption[house_number]/total_power_consumption[house_number], 2)}')

# st.write(f'Carbon emissions due to heating: {np.round(total_grid_power_consumption[house_number]*carbon_intensity[house_number], 2)} g')
# st.write(f'Total bang bang power consumption: {np.round(total_power_consumption[house_number], 2)} kWh')
# st.write(f'Total bang bang energy consumption cost: {np.round(total_cost_bang_bang[house_number], 2)} €')
# st.write(f'Carbon emissions due to heating: {np.round(bang_bang_total_energy[house_number]*carbon_intensity[house_number], 2)} g')
# st.write(f'Total optimal operation solar consumption: {np.round(total_solar_power_consumption[house_number], 2)} kWh')
# st.write(f'Total bang bang solar consumption: {np.round(solar_bang_bang_consumption[house_number], 2)} kWh')


st.write('Total System Values')
st.write(f'Total heater grid energy consumption: {np.round(np.sum(total_grid_power_consumption), 2)} kWh')
st.write(f'Total energy consumption cost: {np.round(np.sum(energy_consumption_cost), 2)} €')
st.write(f'Total energy consumption: {np.round(np.sum(total_power_consumption), 2)} €')
st.write(f'Solar consumption solar end%: {np.round(100*np.sum(total_solar_power_consumption)/np.sum(solar_production), 2)}')
st.write(f'Solar consumption home end%: {np.round(100*np.sum(total_solar_power_consumption)/np.sum(total_power_consumption), 2)}')

# st.write(f'Carbon emissions due to heating: {np.round(np.sum(total_grid_power_consumption*carbon_intensity), 2)} g')
# st.write(f'Total bang bang energy consumption: {np.round(np.sum(bang_bang_total_energy), 2)} kWh')
# st.write(f'Total bang bang energy consumption cost: {np.round(np.sum(total_cost_bang_bang), 2)} €')
# st.write(f'Carbon emissions due to heating: {np.round(np.sum(bang_bang_total_energy*carbon_intensity), 2)} g')
# st.write(f'Total optimal operation solar consumption: {np.round(np.sum(total_solar_power_consumption), 2)} kWh')
# st.write(f'Total bang bang solar consumption: {np.round(np.sum(solar_bang_bang_consumption), 2)} kWh')

st.write(f'Bang bang time: {time_elapsed}')
